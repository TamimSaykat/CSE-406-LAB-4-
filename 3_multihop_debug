// 3_multihop_debug.ino
#include <painlessMesh.h>
#define   MESH_PREFIX     "MeshLabDemo"
#define   MESH_PASSWORD   "mesh_password123"
#define   MESH_PORT       5555
Scheduler userScheduler; painlessMesh mesh; uint32_t targetId=0;
Task taskHeartbeat( TASK_SECOND * 10, TASK_FOREVER, [](){ String hb=String("HB from ")+mesh.getNodeId(); mesh.sendBroadcast(hb);} );
...
void setup(){ Serial.begin(115200); delay(200);
  mesh.setDebugMsgTypes(ERROR|STARTUP|CONNECTION|COMMUNICATION|S_TIME|GENERAL);
  ...
  userScheduler.addTask(taskHeartbeat); taskHeartbeat.enable(); }
